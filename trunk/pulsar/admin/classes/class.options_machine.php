...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($pulsar_data) || !empty($data)){
			...
			switch ( $value['type'] ) {
				...
				//radiobox option
				case "radio":
					$checked = (isset($pulsar_data[$value['id']])) ? checked($pulsar_data[$value['id']], $option, false) : '';
				...
				*/
				...
				case "color":
					$default_color = '';
					if ( isset($value['std']) ) {
						if ( $pulsar_data[$value['id']] !=  $value['std'] )
							$default_color = ' data-default-color="' .$value['std'] . '" ';
					}
					$output .= '<input name="' . $value['id'] . '" id="' . $value['id'] . '" class="of-color"  type="text" value="' . $pulsar_data[$value['id']] . '"' . $default_color .' />';
				...
				case 'images':
					...
					$select_value = (isset($pulsar_data[$value['id']])) ? $pulsar_data[$value['id']] : '';
				...
				case 'sliderui':
					...
					$s_val  = stripslashes($pulsar_data[$value['id']]);
				...
				case 'switch':
					...
					$output .= '<p class="switch-options">';
						...
						$output .= '<input type="hidden" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="0"/>';
						$output .= '<input type="checkbox" id="'.$value['id'].'" class="'.$fold.'checkbox of-input main_checkbox" name="'.$value['id'].'"  value="1" '. checked($pulsar_data[$value['id']], 1, false) .' />';
				...
				case "media":
					...
					$u_val = '';
					if($pulsar_data[$value['id']]){
						$u_val = stripslashes($pulsar_data[$value['id']]);
					}
